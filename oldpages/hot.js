...
class RecipeList extends React.Component{
  ...
  addEndorsement(event){
    ...
    if(index == -1){
      ...
      fetch(urlname + "/?desiredMethod=ENDORSE&uname=" + temprec).then(response => response.text()).then(data => {
        console.log(data)
        return 1;
      }).then(useless => {fetch(urlname + "/?desiredMethod=HOTALLTIME").then(response => response.text()).then(datas => {
        var data = JSON.parse("[" + datas  + "]")
